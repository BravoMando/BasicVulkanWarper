...
namespace Divine
{
        ...
        const PipelineConfigInfo &configInfo)
    {
        assert(
            ...
            "Can't create graphics pipeline: no pipelineLayout provided in configInfo");
        assert(
            configInfo.renderPass != VK_NULL_HANDLE &&
            "Can't create graphics pipeline: no renderPass provided in configInfo");

        auto vertCode = Pipeline::ReadFile(vertFilePath);
        auto fragCode = Pipeline::ReadFile(fragFilePath);
    ...
    void Pipeline::CreateShaderModule(const std::vector<char> &code, VkShaderModule *pModule)
    {
        ...
        if (vkCreateShaderModule(r_Device.GetDevice(), &createInfo, nullptr, pModule) != VK_SUCCESS)
